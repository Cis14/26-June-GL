namespace Creatio.Copilot
{
	...
	internal interface ICopilotPromptVariableResolver
	{
		...
		/// <summary>
		/// Resolves the variables in the specified prompt with optional overrides.
		/// </summary>
		/// <param name="prompt">The prompt to resolve variables in.</param>
		/// <param name="overrides">Optional dictionary of variable overrides (key format: "Namespace.Name").</param>
		/// <returns>The prompt with resolved variables.</returns>
		/// <exception cref="ArgumentException">Encountered a malformed variable.</exception>
		/// <exception cref="KeyNotFoundException">Encountered an unknown variable without default or override.</exception>
		string Resolve(string prompt, IDictionary<string, string> overrides);
	...
	internal class CopilotPromptVariableResolver : ICopilotPromptVariableResolver
	{
		...
		private static IList<Variable> GetVariables(string text) {
			...
			while ((currentPosition = text.IndexOf(OpenTag, currentPosition, StringComparison.Ordinal)) != -1) {
				...
				string variableNamePart = variableBody;
				int defaultSeparatorIndex = variableBody.IndexOf(DefaultValueSeparator, StringComparison.Ordinal);
				if (defaultSeparatorIndex != -1) {
					variableNamePart = variableBody.Substring(0, defaultSeparatorIndex);
					defaultValue = variableBody.Substring(defaultSeparatorIndex + DefaultValueSeparator.Length);
				}
				string[] variableBodyParts =
					variableNamePart.Split(_variableNameSeparator, 2, StringSplitOptions.RemoveEmptyEntries);
		...
		/// <inheritdoc />
		public string Resolve(string prompt, IDictionary<string, string> overrides) {
			...
			foreach (Variable variable in variables) {
				...
				string value = null;
				string variableKey = $"{variable.Namespace}{_variableNameSeparator[0]}{variable.Name}";
				if (overrides != null && overrides.TryGetValue(variableKey, out string overrideValue)) {
					value = overrideValue;
				} else if (_variableProviders.TryGetValue(variable.Namespace, out IVariableProvider variableProvider)) {
					value = variableProvider.GetVariableValue(variable.Name);
				}
				if (value == null) {
					if (variable.DefaultValue != null) {
						...
						throw new KeyNotFoundException(
							$"Variable '{variableKey}' not found and no default value provided.");
					}
				}
