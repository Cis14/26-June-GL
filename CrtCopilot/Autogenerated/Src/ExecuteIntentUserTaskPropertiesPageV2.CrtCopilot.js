 ...
 */
define("ExecuteIntentUserTaskPropertiesPageV2", ["ExecuteIntentUserTaskPropertiesPageResources",
	"ProcessSchemaUserTaskUtilities", "ProcessUserTaskConstants", "ExecuteIntentUserTaskAngularModule"
	],
	...
	return {
		...
		methods: {
			 ...
			 * @private
			 */
			_getIntentParameterByName: function(parameterName) {
				return this.$IntentParameters.findByAttr("Name", parameterName);
			},

			/**
			 * Adapter method for Angular module contract.
			 * @public
			 * @return {Object}
			 */
			getExecuteIntentAngularAdapter: function() {
				return this;
			},

			getExecuteIntentAngularPanelState: function() {
				return {
					intentSchemaUId: this.get("IntentSchemaUId") || Terrasoft.GUID_EMPTY,
			 ...
			 */
			getExecuteIntentAngularIntentParameters: function(callback, scope) {
				const parameters = [];
				Terrasoft.each(this.$IntentParameters, function(parameter) {
					...
					parameters.push({
						...
						dataValueType: parameter.get("DataValueType"),
						menuItems: parameter.getMenuItemsCollection().mapArrayByAttr("Tag"),
						value: {
							value: value.value,
				...
				callback.call(scope || this, { success: true, parameters });
			},
			 ...
			 * @param {Object} scope Scope.
			 */
			getExecuteIntentAngularIntentFiles: function(callback, scope) {
				const files = [];
				if (!this.$IntentFiles) {
					this.sandbox.subscribe("GetFiles", (files) => {
						this.$IntentFiles = files;
						this.getExecuteIntentAngularIntentFiles(callback, scope);
					}, this, [this.getModuleId("ProcessFilesControlModule")]);
					return;
				}
				Terrasoft.each(this.$IntentFiles, function(parameter) {
					const value = parameter.get("Value");
					files.push({
						direction: parameter.get("Direction"),
						description: parameter.get("Description"),
						name: parameter.get("Name"),
						caption: '',
						dataValueType: Terrasoft.DataValueType.FILE,
						value: {
							value: value.value,
							displayValue: value.displayValue
						}
				...
				callback.call(scope || this, files);
			},
			 ...
			 * @param {string} parameterName
			 */
			getIntentParameterValue: function (parameterName) {
				const parameter = this._getIntentParameterByName(parameterName);
				return parameter ? parameter.get("Value") : null;
			},
			 ...
			 * @param {string} parameterName
			 */
			getFileParameterValue: function (parameterName) {
				const parameter = this.$IntentFiles.findByAttr("Name", parameterName);
				return parameter ? parameter.get("Value") : null;
			},
			 ...
			 * @param parameterName
			 */
			clearIntentParameterValue: function(parameterName) {
				const parameter = this._getIntentParameterByName(parameterName);
				if (parameter) {
					parameter.clearParameterValue(parameter.name);
				}
			},

			clearIntentFilesValue: function(fileName) {
				const file = this.$IntentFiles.findByAttr("Name", fileName);
				if (file) {
					file.clearParameterValue(file.name);
				}
			},
			 ...
			 */
			openFormulaEditor: function(parameterName, mappingType) {
				const parameter = this.$IntentParameters.findByAttr("Name", parameterName) || this.$IntentFiles.findByAttr("Name", parameterName);
				const parameterMenuItems = parameter.getMenuItemsCollection();
				const menuItem = parameterMenuItems.findByFn(item => item.$Tag?.mappingType === mappingType)
					// TODO #ENG-86539 Remove this fallback when mapping type support is merged to monorepo
					?? parameterMenuItems.findByAttr("MarkerValue", "FormulaMenuItem");
				if (menuItem) {
					parameter.onMenuItemClick(menuItem.$Tag);
					return true;
				} else {
					return false;
				}
			 ...
			 * Adapter method for Angular module contract.
			 * @public
			 * @param {String} intentSchemaUId Intent schema UId.
			 * @param {Function} callback Callback.
			 * @param {Object} scope Scope.
			 */
			applyExecuteIntentAngularSchemaChanged: function(intentSchemaUId, callback, scope) {
				this.get("IntentSchemaList")?.clear();
				this._initIntentSchemaList(()=> {
					this._loadIntentSchemaLookupValue(intentSchemaUId, function(lookupValue) {
						this.set("IntentSchemaUId", intentSchemaUId);
						this.set("IntentSchema", lookupValue || null, {
							silent: true
						});
						this.set("ShowIntentErrorMessage", false);
						this.onAfterSchemaChanged(callback, scope);
					}, this);
				});
			},

			createFile: function() {
				const moduleId = this.getModuleId("ProcessFilesControlModule");
				this.sandbox.publish("CreateFile", null, [moduleId]);
			},
			 ...
			 * @override
			 */
			onAfterSchemaChanged: function(callback, scope) {
			...
			onItemFocused: function() {
				this.callParent(arguments);
				this.sandbox.publish("ExecuteIntentPropertiesPageFocused", null, [this.sandbox.id]);
			},
					...
					Terrasoft.chain(
						...
						function(next) {
							...
							this._getIntentParameters(schemaUId, function(intentSchemaInfo) {
								...
								this.trySetParameterMappingValue(element, "CopilotWorkflowRootSessionId", "CopilotRootSessionId");
								this._synchronizeActualSchemaParameters(element, schemaUId, intentSchemaInfo || {});
			...
			init: function(callback, scope) {
				this.callParent([function() {
					...
					this.sandbox.subscribe("GetExecuteIntentAngularAdapter", this.getExecuteIntentAngularAdapter, this, [this.sandbox.id]);
					this.sandbox.subscribe("GetFiles", (files) => (this.$IntentFiles = files), this, [this.getModuleId("ProcessFilesControlModule")]);
					Ext.callback(callback, scope);
				}, this]);
			},
		...
		diff: /**SCHEMA_DIFF*/[
			...
			},
			{
				...
				"values": {
					...
					"wrapClass": ["main-interface-designer-properties-panel"],
					"styles": {
						"height": "100vh"
					},
					"items": [],
			...
			{
				"operation": "insert",
				"name": "ProcessFilesControlModule",
				"parentName": "AngularContainer",
				"propertyName": "items",
				"values": {
			...
			{
				"operation": "insert",
				"name": "ExecuteIntentUserTaskAngularModuleContainer",
				"parentName": "AngularContainer",
				"propertyName": "items",
				"values": {
					"itemType": Terrasoft.ViewItemType.MODULE,
					"moduleName": "ExecuteIntentUserTaskAngularModule",
				}
			},
		]/**SCHEMA_DIFF*/
	};
