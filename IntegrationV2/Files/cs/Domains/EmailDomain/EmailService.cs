namespace Terrasoft.EmailDomain
{
	...
	public class EmailService : IEmailService
	{
		...
		private List<AttachmentModel> CreateAttachments(Email emailDto, out string fixedBody) {
			...
			foreach (var attach in emailDto.Attachments) { 
				...
				if (attach.IsInline) {
					...
					if (attach.Id.IsNotNullOrEmpty() && fixedBody.Contains(cidUrl)) {
						fixedBody = fixedBody.Replace(cidUrl, url);
					} else {
						_log.Info($"Attachment name: {attachModel.Name} with cId: {cidUrl} not found in message body.");
		...
		/// <param name="ignoreRights">Flag to ignore rights.</param>
		private void UpdateAttacmentsInlineFlag(Email email, bool ignoreRights) {
			foreach (var attachment in email.Attachments) {
				if (email.Body.Contains("cid:" + attachment.Id)) {
					_attachmentRepository.SetInline(Guid.Parse(attachment.Id), ignoreRights);
				}
			}
		}
		...
		/// <inheritdoc cref="IEmailService.GetEmailMessageData(Email, Guid, string))"/>
		public Entity GetEmailMessageData(Email email, Guid mailboxId = default(Guid), string syncSessionId = null) {
			var subject = email.Subject;
			_log.Trace($"SessionId: {syncSessionId}. Creating Email model for mailbox: {mailboxId}. Email subject: {subject}");
			var emailModel = CreateEmailModel(email);
			_log.Trace($"SessionId: {syncSessionId}. Created Email model for mailbox: {mailboxId}. Email subject: {subject}");
			return _emailRepository.GetEmailMessageData(emailModel, mailboxId, syncSessionId);
		}

		/// <inheritdoc cref="IEmailService.Save(Email, Guid, string))"/>
		...
		/// <inheritdoc cref="IEmailService.Send(Email, Credentials, bool, int)"/>
		public string Send(Email email, Credentials credentials, bool ignoreRights = false, int apiVersion = 2) {
			...
			UpdateAttacmentsInlineFlag(email, ignoreRights);
			return emailProvider.Send(email, email.Attachments, credentials, apiVersion);
		}
