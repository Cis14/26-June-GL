namespace Terrasoft.Configuration.FileUpload
{
	...
	public class FileUploader : FileHandler
	{
		...
		private const string DisableFileuploadChekerFeatureName = "DisableFileuploadCheker";
		...
		private static bool GetCanUseAdminRights(IFileUploadInfo fileInfo, UserConnection userConnection) {
			...
			if (extraParams != null && userConnection != null &&
					...
					result is bool useAdminRights) {
				if (useAdminRights) {
					return true;
				}
				if (userConnection.GetIsFeatureEnabled(DisableFileuploadChekerFeatureName)) {
					return false;
				}
				return !CheckFileUploadRights(fileInfo, userConnection);
			}
			if (fileInfo.EntitySchemaName == FileConsts.FeedFileEntitySschemaName) {
				return !CheckSocialMessageAuthor(fileInfo, userConnection);
			}
			return true;
		}

		private static bool CheckFileUploadRights(IFileUploadInfo fileInfo, UserConnection userConnection) {
			if (!ClassFactory.TryGet(fileInfo.EntitySchemaName, out IFileUploadRightsChecker fileUploadRightsChecker)) {
				return false;
			}
			return fileUploadRightsChecker.CheckRights(userConnection, fileInfo.FileId);
		}

		private static bool CheckSocialMessageAuthor(IFileUploadInfo fileInfo, UserConnection userConnection) {
			if (fileInfo.ParentColumnValue == null || fileInfo.ParentColumnValue.IsEmpty()) {
				return false;
			}
			Select select = new Select(userConnection).Top(1)
				.Column("Id")
				.From("SocialMessage")
				.Where("Id").IsEqual(Column.Parameter(fileInfo.ParentColumnValue))
		...
		private void Upload(IFileUploadConfig fileUploadConfig) {
			...
			IFile file = isFirstChunkOrSinglePart
				...
				: LoadFileParts(fileFactory, fileUploadInfo, fileLocator);
		...
		private IFile LoadFileParts(IFileFactory fileFactory, IFileUploadInfo fileUploadInfo,
				EntityFileLocator fileLocator) {
			fileFactory.UseRights = GetUseRightsForLoadFileParts(fileUploadInfo);
			return fileFactory.Get(fileLocator);
		}

		private bool GetUseRightsForLoadFileParts(IFileUploadInfo fileUploadInfo) {
			if(UserConnection.GetIsFeatureEnabled("DisableCheckParentEntityExistsForFileUpload")) {
				return GetCanUseAdminRights(fileUploadInfo, UserConnection);
			}
			if (fileUploadInfo?.AdditionalParams == null || 
				!fileUploadInfo.AdditionalParams.TryGetValue("RecordSchemaName", out object parentSchemaValue)) {
				return GetCanUseAdminRights(fileUploadInfo, UserConnection);
			}
			string parentSchemaName = parentSchemaValue?.ToString() ?? string.Empty;
			if (parentSchemaName.IsNullOrEmpty()) {
				return GetCanUseAdminRights(fileUploadInfo, UserConnection);
			}
			IEntityFactory entityFactory = ClassFactory.Get<IEntityFactory>();
			IEntity parentEntity = entityFactory.CreateEntity(parentSchemaName);
			if (!parentEntity.FetchFromDB(fileUploadInfo.ParentColumnValue, false)) {
				return false;
			}
			return GetCanUseAdminRights(fileUploadInfo, UserConnection);
		}
